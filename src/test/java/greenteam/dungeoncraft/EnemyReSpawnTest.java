...
public class EnemyReSpawnTest {
	
	@Before
	public void setUpBeforeClass_CreateContext() throws Exception {
		GameEngine.Init();
		new Main();
		//initialize player as the player recieves a score when the enemy is killed (just before respawning)
		 Main.getPlayer().init();
	}

	@Test /*test the respawn of the skull enemy*/
	public void enemyRespawnTest() {
		...
		//check to see if the enemy is now inactive due to reaching it's maxium amount of respawns
		//assert(skullEnemy.isActive() == false);
